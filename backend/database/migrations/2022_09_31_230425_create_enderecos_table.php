...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('tb_endereco', function (Blueprint $table) {
            ...
            $table->foreign('codigo_pessoa')->references('codigo_pessoa')->on('tb_pessoa')->onUpdate('cascade')
                ->onDelete('cascade');

            $table->unsignedBigInteger('codigo_bairro');

            $table->foreign('codigo_bairro')->references('codigo_bairro')->on('tb_bairro')->onUpdate('cascade')
                ->onDelete('cascade');
        });
    }
